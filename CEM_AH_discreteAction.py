...
def sample_realCEM_eObs_netBatch_mini_msCEM(theta_mean, theta_std, msCEM_model, all_obs_v_buffer, obs_size, n_actions, net_batch, show=True):
    if show:
        print('======================== Generating real CEM action probabilities ========================')
    start_time = time.time()

    # empty tensors initialise
    CEM_obs_v = torch.zeros(0, obs_size)
    CEM_act_probs_v = torch.zeros(0, n_actions)

    if use_cuda:
        CEM_obs_v = CEM_obs_v.to(device)
        CEM_act_probs_v = CEM_act_probs_v.to(device)

    thetas = np.vstack([np.random.multivariate_normal(theta_mean, np.diag(theta_std ** 2)) for _ in range(net_batch)])

    batch_size = max(1, math.ceil(all_obs_v_buffer.shape[0] / net_batch))
    # construct a batch generator
    dataset = torch.utils.data.TensorDataset(all_obs_v_buffer)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)

    for i, data in enumerate(dataloader, 0):
        # set the current model's generated weights
        theta_tensor = torch.FloatTensor(thetas[i].flatten())
        loadWeight_dict = utils.getLoadWeightDict(msCEM_model.state_dict(), theta_tensor)
        msCEM_model.load_state_dict(loadWeight_dict)

        CEM_obs_v = torch.cat((CEM_obs_v, data[0]), 0)

        model_out = model(data[0])
        model_out[torch.isnan(model_out)] = 0
        act_probs_v = sm(model_out)

        CEM_act_probs_v = torch.cat((CEM_act_probs_v, act_probs_v), 0)

    elapsed_time = time.time() - start_time
    if show:
        print("CEM tuples=%d, \tDuration:%s "
              % (CEM_obs_v.shape[0], time.strftime("%H:%M:%S", time.gmtime(elapsed_time))))

    return CEM_obs_v, CEM_act_probs_v
...
# create real dataloader for Discriminator
...
if __name__ == '__main__':
    ...
    if LAYER == 2:   # 2-layer network
        # model = models.MainNetW_eB_SingleLayer(args, obs_size, n_actions)
    ...
    if LAYER == 3:  # 3-layer network
        model = models.MainNetW_eB(args, obs_size, n_actions)
    ...
    for gan_epoch in range(gan_epochs):
        ...
        # CEM_obs_v, CEM_act_probs_v = sample_realCEM_eObs_netBatch_mini_msCEM(theta_mean, theta_std, msCEM_model,
        #                                                                      all_obs_v_buffer, obs_size, n_actions,
        #                                                                      NET_BATCH)
        # all_obs_v_buffer = CEM_obs_v
        # all_act_probs_v_buffer = CEM_act_probs_v
