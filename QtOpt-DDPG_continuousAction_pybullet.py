...
class QT_Opt(nn.Module):
    def __init__(self, replay_buffer, hidden_dim, action_lim, q_lr=3e-4, cem_update_itr=2, select_num=6, num_samples=64):
        ...
        if LAYER == 2:
            ...
            self.actor = models.Actor_DDPG_2Layer(args, obs_size, n_actions, action_lim).to(device)
            self.target_actor = models.Actor_DDPG_2Layer(args, obs_size, n_actions, action_lim).to(device)
        ...
        if LAYER == 3:
            ...
            self.actor = models.Actor_DDPG_3Layer(args, obs_size, n_actions, action_lim).to(device)
            self.target_actor = models.Actor_DDPG_3Layer(args, obs_size, n_actions, action_lim).to(device)
        ...
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=q_lr)
    ...
    def update(self, batch_size, gamma=0.9, soft_tau=1e-2, update_delay=50):
        ...
        actor_action = self.actor(state_batch).to(device)
        predict_actor_q = self.qnet(state_batch, actor_action)  # predicted Q(s, Pi(s)) value

        cgp_loss = ((actor_action - action_batch.detach()) ** 2).mean()
        qgp_loss = -1 * torch.sum(predict_actor_q)
        actor_loss = cgp_loss + qgp_loss

        cgp_loss_value = cgp_loss.item()
        qgp_loss_value = qgp_loss.item()
        ...
        # update target actor
        self.target_actor = self.target_soft_update(self.actor, self.target_actor, soft_tau)
        return q_loss_value, cgp_loss_value, qgp_loss_value, actor_loss_value
    ...
    def load_model(self, path):
        ...
        self.target_actor.load_state_dict(torch.load(path))
...
def load_args():
    ...
    parser.add_argument('--write_dir', default='QtOpt-DDPG', type=str)
...
if __name__ == '__main__':
    ...
    DEVICE = args.device
    # monitor_directory = 'monitor_QtOpt-DDPG'
    model_directory = 'model_QtOpt-DDPG'
    ...
    for epoch in range(EPOCHS):
        ...
        for state_num, agent_start in enumerate(agent_start_list):
            ...
            for episodes in range(eval_ep_batch):
                ...
                while True:
                    ...
                    action = qt_opt.target_actor(obs_v)  # uses the actor policy for test

                    next_obs, reward, is_done, _ = env.step(action.flatten())
                    episode_reward += reward
                    # action_v = torch.tensor([action], device=device).reshape(1, -1)
                    action_v = action.float()
        ...
        for state_num, agent_start in enumerate(agent_start_list):
            ...
            for episodes in range(1):
                ...
                while True:
                    # Select and perform an action
                    action = qt_opt.cem_optimal_action_explore(obs_v)
                    ...
                    replay_buffer.push(obs_v, action_v, reward_v, next_obs_v, done)
                ...
                if len(replay_buffer) >= BATCH_SIZE:
                    q_loss_value, cgp_loss_value, qgp_loss_value, actor_loss_value = qt_opt.update(BATCH_SIZE)
        ...
        elapsed_time = time.time() - start_time
        print(
            "epoch: %d, \tq_loss: %.4f, \tactor_loss: %.4f, \treward_accum: %.4f, \treward_mean: %.4f, \treward_std: %.4f, \tsteps: %d, \tratio: %.4f, \tElapsed:%s" % (
                epoch, q_loss_value, actor_loss_value, reward_accum, reward_m, reward_std, step_counts_m, r_ratio,
                time.strftime("%H:%M:%S", time.gmtime(elapsed_time))))
        ...
        torch.save(qt_opt.actor.state_dict(), model_path + '_actor' + '.pth')
        torch.save(qt_opt.target_actor.state_dict(), model_path + '_target_actor' + '.pth')

        writer.add_scalar("loss/q_loss", q_loss_value, epoch)
        writer.add_scalar("loss/cgp_loss", cgp_loss_value, epoch)
        writer.add_scalar("loss/qgp_loss", qgp_loss_value, epoch)
        writer.add_scalar("loss/actor_loss", actor_loss_value, epoch)
